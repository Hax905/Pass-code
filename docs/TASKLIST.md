...
- [2026-09-21] [Completed] **Two plain-text guides for people, not sessions:** `HOW-PASSCODE-WORKS.txt` (overview: problem, architecture, data model, each subsystem, the security properties each paired with the mechanism that enforces it, how to run it, tradeoffs) and `PASSCODE-FUNCTIONALITY.txt` (in depth: every index and the query it backs, all four transactional paths, the error taxonomy, the client layer, and nine workflows traced end to end through code, database writes and HTTP results). Both record the limits honestly, including one found while writing them: `getCurrentNetworkPassword()` is called inside the disclosure transaction but reads without the session, so that read is not part of the transaction snapshot — harmless in practice, but a real asymmetry
